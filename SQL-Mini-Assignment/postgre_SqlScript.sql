...
select account_id, cust_id, avail_balance, status from account where avail_balance > 2500.00 and status = 'ACTIVE';

Select p.product_cd, a.cust_id, a.avail_balance from product p inner join account a on p.product_cd = a.product_cd 
where lower(p.product_type_cd) = 'account'
